...
public:
    ...
    Node *solve(int &index,int size,int pre[],int maxValue)
    {
        if(index==size || pre[index]>=maxValue) return NULL;
        Node * root=newNode(pre[index++]);
        root->left=solve(index,size,pre,root->data);
        root->right=solve(index,size,pre,maxValue);
        return root;
    }
    Node* post_order(int pre[], int size)
    {
        int index=0;
        return solve(index,size,pre,INT_MAX);
    }
};
